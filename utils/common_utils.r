...
COMMONUTILS_load_dataframe <- function(filename, factors = FALSE) {
...
COMMONUTILS_write_dataframe <- function(df, filename) {
  write.csv(df, file(filename, encoding = "UTF-8"),row.names=FALSE)
}

COMMONUTILS_geocode_yandex <- function(location) {
  location <- URLencode(location, reserved = TRUE, repeated = FALSE)
  json_res <- HTTP_make_http_get_request_plain(paste("https://geocode-maps.yandex.ru/1.x/?format=json&geocode=", location, sep=""))
